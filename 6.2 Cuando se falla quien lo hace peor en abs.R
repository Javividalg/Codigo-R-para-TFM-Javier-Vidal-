...
#A. Crear una matriz de dos columnas la primera con el error de r y la segunda con el error absoluto de la consultora
#Generamos una matriz 1000x2 la primera columna muesta el error abs cuando el modelo propuesto falla y 0 cuando acierta
#la segunda el error abs del modelo de la consultora cuando esta falla y 0 cuando acierta
...
for (i in 1:nro.trayectorias) {
  ...
  if (dif.mb.consultora.trayectorias[i] < dif.mb.suavizada2.trayectorias[i])
  {
    ...
    error[i,1]<-dif.mb.suavizada2.trayectorias[i]
    error[i,2]<-0
    
  } else{
    error[i,1]<-0
    error[i,2]<-dif.mb.consultora.trayectorias[i]
  }
}
error.r<-error[,1]
error.co<-error[,2]
...
hist(error.r[error.r>0],breaks = seq(xmin, xmax, ancho_intervalo),freq=FALSE,col = alpha("blue", 0.5),xlab = "valores", ylab = "Densidad", main = "Error absoluto",cex.main = 2,cex.lab=1.5)
hist(error.co[error.co>0],breaks = seq(xmin, xmax, ancho_intervalo),freq=FALSE, add=TRUE, col = alpha("red", 0.5))
a<-quantile(error.r[error.r>0],c(0.5))
